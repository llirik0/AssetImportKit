...
class PreviewManager: NSObject, QLPreviewControllerDataSource {
    ...
    func previewViewControllerForFile(_ file: FBFile, fromNavigation: Bool) -> UIViewController {
        ...
        if file.type == .DAE || file.type == .FBX || file.type == .OBJ || file.type == .SCN || file.type == .MD3 || file.type == .ZGL || file.type == .XGL || file.type == .WRL || file.type == .STL || file.type == .SMD || file.type == .RAW || file.type == .Q3S || file.type == .Q3O || file.type == .PLY || file.type == .XML || file.type == .MESH || file.type == .OFF || file.type == .NFF || file.type == .M3SD || file.type == .MD5ANIM || file.type == .MD5MESH || file.type == .MD2 || file.type == .IRR || file.type == .IFC || file.type == .DXF || file.type == .COB || file.type == .BVH || file.type == .B3D || file.type == .AC || file.type == .BLEND || file.type == .HMP || file.type == ._3DS || file.type == ._3D || file.type == .X || file.type == .TER || file.type == .MAX || file.type == .MS3D || file.type == .MDL || file.type == .ASE {
