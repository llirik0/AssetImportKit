...
//        let popoverSize = CGSize(width: popoverWidth, height: popoverHeight)
    ...
    @IBAction func rightButtonAction(_ sender: Any) {
        ...
        if VirtualObject.availableObjects.count == 0 {
            ...
            if self.virtualObject == nil {
                self.virtualObject = VirtualObject.availableObjects[0]
            } else {
                for objectIndex in 0 ... VirtualObject.availableObjects.count - 1 {
                    if VirtualObject.availableObjects[objectIndex].modelName == self.virtualObject?.modelName {
                        selectedObjectIndex = objectIndex
                    }
                }
            }
        }
        if selectedObjectIndex == VirtualObject.availableObjects.count - 1 {
            loadVirtualObject(at: 0)
        } else {
            loadVirtualObject(at: selectedObjectIndex + 1)
        }
    ...
    @IBAction func leftButtonAction(_ sender: Any) {
        ...
        if selectedObjectIndex == 0 {
            loadVirtualObject(at: VirtualObject.availableObjects.count - 1)
        } else {
            loadVirtualObject(at: selectedObjectIndex - 1)
        }
